<form class="form-validate-upd-jquery modifier-bc-content form-facture" id="bc_form_upd" action="<?= base_url('BonDeCommande/majBc') ?>" method="post">
    ...
    <?php if ($disabled == ""): ?>
        <div class="modal-footer d-flex justify-content-end" id="div-upd-footer">
            <button type="button" class="btn btn-success btn-sm float-right" style="background-color:#21a89f;" id="valider" onclick="validerFacture('<?= $arr_bc->id ?>','3')"
                data-loading-text="<i class='icon-spinner10 spinner'></i> Valider" <?= $disabled; ?>>
                Valider</button>
            <button type="button" class="btn btn-danger btn-sm float-right" id="refuser" onclick="validerFacture('<?= $arr_bc->id ?>','2')"
                data-loading-text="<i class='icon-spinner10 spinner'></i> Refuser" <?= $disabled; ?>>
                Refuser</button>
