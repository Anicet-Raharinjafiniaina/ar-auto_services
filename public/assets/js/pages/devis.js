$(function () {
    ...
    initialiseSelect2Modal("article_id0", "modal_ajout_devis")
    $("#autre_titre").hide()
    let index_article = 1;
    let index_autre = 0;
    $('#add_article').on('click', function () {
        ...
        $('#article_table tbody').append(`
            <tr>
                ...
                <td><input class="form-control input-xs text-end obligatoire" id="quantite${index_article}" name="article[${index_article}][quantite]" type="text" onkeyup="formatMontant(this)">
        ...
        `);
        initialiseSelect2Modal("article_id" + index_article, "modal_ajout_devis")
        toggleRemoveButtons()
        index_article++;
    });
    ...
    $("#btn-create-devis").click(function () {
        ...
        $("#modal_ajout_devis").modal("show");
        $('#type_client, #client, #article_id').select2({
            dropdownParent: $('#modal_ajout_devis .modal-body')
        });
        loadVehicule()
    });

    $('#modal_ajout_devis .modal-body').on('scroll', function () {
    ...
    $('#modal_ajout_devis').on('shown.bs.modal', function () {
        ...
        setTimeout(() => {
            ...
            $('#modal_ajout_devis .select-search').each(function () {
            ...
            initialiseSelect2ModalByClass("select-search", "modal_ajout_devis");
...
$('#type_client').on('change', function () {
    loaderContent('modal_ajout_devis')
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            stopLoaderContent('modal_ajout_devis')
        }
    })
})
...
function prixAuto(e) {
    ...
    loaderContent('modal_ajout_devis')
    $.ajax({
        url: urlProject + 'Devis/getPrixArticle',
        type: 'POST',
        dataType: 'json',
        data: {
...
$('#devis_form').on('submit', function (e) {
    ...
    $(".validation-error-label").html("");
    isValid = checkObligatoire(".devis-form", ".obligatoire")
    if (isValid == true) {
        loaderContent('modal_ajout_devis')
        ...
        $.ajax({
            ...
            dataType: 'json',
            success: function (id) {
                var url = urlProject + "Devis/viewOrDownloadPDF?id=" + encodeURIComponent(id) + "&action=view";
                window.open(url, '_blank');
                stopLoaderContent('modal_ajout_devis')
                $("#save").prop("disabled", false);
                Swal.fire({
                    title: "Création",
                    html: "Création devis faite avec succès.",
                    icon: "success",
...
function view(id, action) {
    loaderContent('main')
    const url = urlProject + `Devis/viewOrDownloadPDF?id=${id}&action=${action}`;
    window.open(url, '_blank');
...
function deleteDevis(id) {
    Swal.fire({
        ...
        text: "La suppression de ce devis est irréversible !",
        icon: "warning",
        ...
        preConfirm: () => {
            ...
            return $.ajax({
                type: "POST",
                url: urlProject + "Devis/deleteDevis",
                data: { id: id },
